...
class Logger {
    constructor() {
        this.map = new Map();
        this.pBar = new bar.SingleBar({ format: `progress [ ${colors_1.cyan('{bar}')} {percentage}% ] | {task}` }, bar.Presets.rect);
        this.total = 0;
    }
    ...
    updateBar(num, payload, type) {
        ...
        switch (type) {
            case 'inc':
                ...
                break;
            case 'set':
                this.pBar.update(num, { task: payload });
                this.total = num;
        }
    }
}
