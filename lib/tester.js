...
const LoadTester = ({ apolloConfig, scenario }) => {
    ...
    for (let i = 0; i < repeat; i++) {
        threads_1.spawn(new threads_1.Worker("./workers/processScenario.ts"))
            .then(w => //  @ts-ignore
         w.processScenarioParallel({ scenario, uri: uri, apolloConfig })
            .then((value) => {
...
const runRequest = (request, type, stepName, client) => {
    ...
    return new Promise((resolve => {
        gqlRequest()
            ...
            .catch(e => {
            const time = calculateTimeElapsed(initialTime);
            const log = { stepName, callName: name, time, type: 'failed' };
